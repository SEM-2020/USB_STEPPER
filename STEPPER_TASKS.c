 ...
 */
 void vInit_Steppers_Tasks()
 {
     ...
     static stepper_params brakes_structure;


     /* Queue initialisation */

     Queue_steering = xQueueCreate( 1,2);// single element queue , 2 bytes in size
     Queue_throtle = xQueueCreate( 1,2);
     Queue_brakes = xQueueCreate( 1,2);
     ...
     //throtle
     throtle_structure.Queue_steps_desired=Queue_throtle;
     throtle_structure.driver_port_clock=THROTLE_DRIVER_PORT_CLOCK;
     throtle_structure.driver_port_base=THROTLE_DRIVER_PORT_BASE;
     throtle_structure.driver_direction_pin=THROTLE_DIRECTION_PIN;
     throtle_structure.driver_enable_pin=THROTLE_ENABLE_PIN;
     throtle_structure.driver_pulse_pin=THROTLE_PULSE_PIN;
     throtle_structure.driver_delay=THROTLE_STEP_DELAY;

     //brakes
     brakes_structure.Queue_steps_desired=Queue_brakes;
     brakes_structure.driver_port_clock=BRAKES_DRIVER_PORT_CLOCK;
     brakes_structure.driver_port_base=BRAKES_DRIVER_PORT_BASE;
     brakes_structure.driver_direction_pin=BRAKES_DIRECTION_PIN;
     brakes_structure.driver_enable_pin=BRAKES_ENABLE_PIN;
     brakes_structure.driver_pulse_pin=BRAKES_PULSE_PIN;
     brakes_structure.driver_delay=BRAKES_STEP_DELAY;
     ...
     xTaskCreate( vTask_Stepper ,
                  "throtle_task" ,                       // Name of the task for later debugging
                  STEPPER_vTASK_STACK_DEPTH ,                  // size of the stack.
                  &throtle_structure ,                      // task parameters.
                  configMAX_PRIORITIES-THROTLE_vTASK_PRIO ,  // priority is inverted in freeRTOS.
                  NULL ) ;            // handle of the task.

     xTaskCreate( vTask_Stepper ,
                  "brakes_task" ,                       // Name of the task for later debugging
                  STEPPER_vTASK_STACK_DEPTH ,                  // size of the stack.
                  &brakes_structure ,                      // task parameters.
                  configMAX_PRIORITIES-BRAKES_vTASK_PRIO ,  // priority is inverted in freeRTOS.
                  NULL ) ;            // handle of the task.
 ...
 void vTask_Stepper(void * para)
 {
     ...
     int16_t steps_current = 0;
     int16_t  steps_desired = 0;
